...
// an array of size:6 with Xmin, Ymin, Zmin and Xmax, Ymax, Zmax values
    ...
    void Mesh::getAttribData(const std::string &descriptor,
                             std::vector<GLAttributeMapping> &bindings, int &total_size) {
        ...
        bindings.push_back(binding);
#endif
        if (descriptor.find("a_normal") != std::string::npos ||
            descriptor.find("normalTexture") != std::string::npos) {
...
// call this from renderCamera, get attribute descriptor from shader
    void Mesh::generateVKBuffers(const std::string &descriptor, VulkanRenderer *renderer) {
        if (!vao_dirty_)
            return;
        int total_size = 0;
        VkDevice &m_device = renderer->getDevice();
        VulkanCore *vulkanCore = renderer->getCore();
        ...
        err = vkCreateBuffer(m_device, gvr::BufferCreateInfo(buffer.size() * sizeof(float),
        ...
        pass = vulkanCore->GetMemoryTypeFromProperties(mem_reqs.memoryTypeBits,
    ...
    void Mesh::createAttributeMapping(int programId, int& totalStride, int &attrLen) {
        ...
        for (int i = 0; i < numActiveAtributes; i++) {
            ...
            if (std::find(dynamicAttribute_Names_.begin(), dynamicAttribute_Names_.end(),
                          attrName) != dynamicAttribute_Names_.end()) {
    ...
    const GLuint Mesh::getVAOId(int programId) {
        ...
        return generateVAO(programId);
    }

// generate vertex array object
    int Mesh::generateVAO(int programId) {
        ...
        if (0 != normals_.size() && vertices_.size() != normals_.size()) {
            LOGW("mesh: number of vertices and normals do not match! vertices %d, normals %d",
        ...
        if (it != program_ids_.end()) {
            ...
            vao_dirty_ = true;
            checkGLError("Mesh:generateVAO create buffers");
        }

        glBindVertexArray(vaoID_);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, triangle_vboID_);
        glBindBuffer(GL_ARRAY_BUFFER, static_vboID_);
        checkGLError("Mesh:generateVAO bind buffers");
        if (vao_dirty_)
        {
            glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned short) * indices_.size(),
        ...
        for (std::vector<GLAttributeMapping>::iterator it = attrMapping.begin();
             it != attrMapping.end(); ++it) {
            GLAttributeMapping currData = *it;
            glVertexAttribPointer(currData.index, currData.size, currData.type, 0,
                                  totalStride_ * sizeof(GLfloat),
                                  (GLvoid *) (currData.offset * sizeof(GLfloat)));
    ...
    void Mesh::generateBoneArrayBuffers(GLuint programId) {
        if(!bone_data_dirty_ && boneVboID_ !=0)
            return;
        ...
        if (bone_data_dirty_)
        {
            ...
            bone_data_dirty_ = false;
        }
        glEnableVertexAttribArray(getBoneIndicesLoc());
        glVertexAttribIPointer(getBoneIndicesLoc(), 4, GL_INT, sizeof(VertexBoneData::BoneData),
                               (const GLvoid *) 0);

        // BoneWeight
        glEnableVertexAttribArray(getBoneWeightsLoc());
        glVertexAttribPointer(getBoneWeightsLoc(), 4, GL_FLOAT, GL_FALSE,
                              sizeof(VertexBoneData::BoneData),
                              (const GLvoid *) (sizeof(VertexBoneData::BoneData::ids)));

        //glBindVertexArray(0);
        //glBindBuffer(GL_ARRAY_BUFFER, 0);
        LOGV("SHADER: create bone array buffers %d", boneVboID_);
    }
