...
namespace gvr
{
    ...
    GLImage* Light::depth_texture_ = NULL;
 ...
 */
    void Light::bindShadowMap(Shader *shader, int texIndex)
    {
        GLShader *glshader = reinterpret_cast<GLShader *>(shader);
        int program = glshader->getProgramId();
        ...
        {
            LOGD("LIGHT: found shadow map in shader at loc %d\n", loc);
    ...
    bool Light::generateFBO()
    {
        ...
        if (fboStatus != GL_FRAMEBUFFER_COMPLETE)
        {
            ...
            switch (fboStatus)
            {
                case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT : LOGE(
                            "GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT");
                    break;
                case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: LOGE(
                            "GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT");
                    break;

                case GL_FRAMEBUFFER_UNSUPPORTED:LOGE("GL_FRAMEBUFFER_UNSUPPORTED");
                    break;
            }
            return false;
        }
    ...
    {
        GLImage *depthtex = new GLImage(GL_TEXTURE_2D_ARRAY);
        if (!depthtex->updateGPU())
        {
            LOGE("texture creation error");
            return;
        }
        depth_texture_ = depthtex;
        glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA8, width, height, depth, 0, GL_RGBA,
                     GL_UNSIGNED_BYTE, NULL);
        glBindTexture(GL_TEXTURE_2D_ARRAY, 0);
        checkGLError("Light::createDepthTexture");
    #ifdef DEBUG_LIGHT
        LOGD("LIGHT: create shadow map depth texture");
    #endif
    }
    ...
    void Light::deleteDepthTexture()
    {
        if (depth_texture_)
        {
            GLuint id = dynamic_cast<GLImage*>(depth_texture_)->id();
            if (id)
            {
    #ifdef DEBUG_LIGHT
 ...
 */
    bool Light::makeShadowMap(Scene *scene, ShaderManager *shader_manager, int texIndex,
                              std::vector<SceneObject *> &scene_objects, int viewport_width,
                              int viewport_height)
    {
